...
namespace InfiniFrame.Window.Features.State;
// ---------------------------------------------------------------------------------------------------------------------
// Code
// ---------------------------------------------------------------------------------------------------------------------
internal sealed class StateWebMessageDispatcher : WindowFeatureWebMessageDispatcherBase<IStateInfiniFrameWindowFeature> {
    ...
    protected override void Post(IStateInfiniFrameWindowFeature feature, string command, JsonElement? args) {
        switch (command) {
            case "setCachedPreFullScreenBounds":
                feature.CachedPreFullScreenBounds = Required<Rectangle>(args, "bounds");
                return;
            case "setCachedPreMaximizedBounds":
                feature.CachedPreMaximizedBounds = Required<Rectangle>(args, "bounds");
                return;
            case "setMaximized":
