...
Photino::Photino(PhotinoInitParams* initParams)
{
    ...
    {
        wchar_t msg[200];
        swprintf(msg, 200, L"Initial parameters passed are %i bytes, but expected %I64i bytes.", initParams->Size,
    ...
    if (initParams->Chromeless)
    {
        ...
        if (initParams->Left == CW_USEDEFAULT && initParams->Top == CW_USEDEFAULT) initParams->CenterOnInitialize =
            true;
    ...
    if (initParams->Height < initParams->MinHeight && initParams->MinHeight > 0) initParams->Height = initParams->
        MinHeight;
...
{
    HICON iconSmall = (HICON)LoadImage(NULL, filename, IMAGE_ICON, 16, 16,
                                       LR_LOADFROMFILE | LR_LOADTRANSPARENT | LR_SHARED);
    HICON iconBig = (HICON)LoadImage(NULL, filename, IMAGE_ICON, 32, 32,
                                     LR_LOADFROMFILE | LR_LOADTRANSPARENT | LR_SHARED);
...
void Photino::ShowNotification(AutoString title, AutoString body)
{
    ...
    {
        auto toast = WinToastTemplate(WinToastTemplate::ImageAndText02);
