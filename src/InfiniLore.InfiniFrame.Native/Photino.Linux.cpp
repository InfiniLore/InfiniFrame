...
void Photino::SendWebMessage(AutoString message)
{
    ...
    webkit_web_view_run_javascript(WEBKIT_WEB_VIEW(_webview), js.c_str(), NULL, webview_eval_finished,
                                   &invokeJsWaitInfo);
...
gboolean on_window_state_event(GtkWidget* widget, GdkEventWindowState* event, gpointer self)
{
    ...
    else if (!(event->new_window_state & GDK_WINDOW_STATE_MAXIMIZED) && !(event->new_window_state &
...
void Photino::AddCustomSchemeHandlers()
{
    ...
    for (const auto& value : _customSchemeNames)
    {
        webkit_web_context_register_uri_scheme(
            context, value, (WebKitURISchemeRequestCallback)HandleCustomSchemeRequest, (void*)_customSchemeCallback,
            NULL);
    }
}
