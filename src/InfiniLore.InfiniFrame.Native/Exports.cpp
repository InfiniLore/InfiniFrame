...
//Dialog
EXPORTED AutoString* Photino_ShowOpenFile(Photino* inst, AutoString title, AutoString defaultPath, bool multiSelect,
                                          AutoString* filters, int filterCount, int* resultCount)
{
    return inst->GetDialog()->ShowOpenFile(title, defaultPath, multiSelect, filters, filterCount, resultCount);
}

EXPORTED AutoString* Photino_ShowOpenFolder(Photino* inst, AutoString title, AutoString defaultPath, bool multiSelect,
                                            int* resultCount)
{
    return inst->GetDialog()->ShowOpenFolder(title, defaultPath, multiSelect, resultCount);
}

EXPORTED AutoString Photino_ShowSaveFile(Photino* inst, AutoString title, AutoString defaultPath, AutoString* filters,
                                         int filterCount, AutoString defaultFileName = NULL)
{
    return inst->GetDialog()->ShowSaveFile(title, defaultPath, filters, filterCount, defaultFileName);
}

EXPORTED DialogResult Photino_ShowMessage(Photino* inst, AutoString title, AutoString text, DialogButtons buttons,
                                          DialogIcon icon)
{
...
EXPORTED void InfiniWindowTests_NativeParametersReturnAsIs(const PhotinoInitParams* params,
                                                           PhotinoInitParams** new_params)
{
