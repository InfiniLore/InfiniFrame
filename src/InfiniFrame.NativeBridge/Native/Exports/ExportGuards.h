...
namespace infiniframe::exports {
    ...
    template <typename T>
    inline void ResetOut(T* outValue, const T fallback = {}) noexcept {
        if (outValue != nullptr) {
            *outValue = fallback;
        }
    }
    ...
    template <typename T>
    inline void ResetOut2(T* first, T* second, const T fallback = {}) noexcept {
    ...
    template <typename T, typename Fn>
    inline T RunReturnExport(T fallback, Fn&& fn) noexcept {
        try {
