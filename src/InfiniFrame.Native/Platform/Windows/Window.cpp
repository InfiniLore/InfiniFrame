                        ...
                        HRESULT {
                            ...
                            m_impl->_webviewWindow->
                                    AddScriptToExecuteOnDocumentCreated(
                                        L"window.infiniframe = window.infiniframe || {};window.infiniframe.host = window.infiniframe.host || {};window.infiniframe.host.postMessage = window.infiniframe.host.postMessage || function(envelope) { var message = (typeof envelope === 'string') ? envelope : JSON.stringify(envelope); window.chrome.webview.postMessage(message); };window.external = { sendMessage: function(message) { window.infiniframe.host.postMessage(message); }, receiveMessage: function(callback) { window.chrome.webview.addEventListener('message', function(e) { callback(e.data); }); } };",
                                        nullptr
                                        );
                            m_impl->_webviewWindow->
