...
{
    NSString *initScriptSource = @"window.__receiveCallbackCallbacks = [];"
        ...
        "};"
        "window.__infiniframe.host.getDataAsync = window.__infiniframe.host.getDataAsync || function(message) {"
        ...
        "				else{reject(new Error(payload.error||'Host getDataAsync failed.'));}"
        "			}catch(_){ }"
        "		};"
