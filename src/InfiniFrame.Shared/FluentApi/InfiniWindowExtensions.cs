...
public static class InfiniWindowExtensions {
    extension<T>(T window) where T : class, IInfiniFrameWindow {
        ...
        /// </summary>
        /// <returns>
        /// Returns the current <see cref="IInfiniFrameWindow" /> instance after centering it on the current monitor.
        /// </returns>
        /// <remarks>
        /// This method adjusts the window's position to ensure it is centered on the monitor where the majority of the window resides.
        /// </remarks>
        public T CenterOnCurrentMonitor() => CenterOnMonitor(window, -1);
        /// <summary>
        /// Centers the window on the specified monitor.
        /// </summary>
        /// <param name="monitorIndex">
        /// The index of the monitor on which to center the window.
        /// A value of -1 centers the window on the current monitor.
        /// </param>
        /// <returns>
        /// Returns the current <see cref="IInfiniFrameWindow" /> instance after centering.
        /// </returns>
        ...
        public T MoveWithinCurrentMonitorArea(Point location) => MoveWithinCurrentMonitorArea(window, location.X, location.Y);
        public T MoveWithinCurrentMonitorArea(double left, double top) => MoveWithinCurrentMonitorArea(window, (int)left, (int)top);
        #endregion
        ...
        public T Offset(Point offset) => Offset(window, offset.X, offset.Y);
        public T Offset(double left, double top) => Offset(window, (int)left, (int)top);
        #endregion
        ...
        public T SetSize(Size size) => SetSize(window, size.Width, size.Height);
        #endregion
        ...
        public T SetLocation(Point location) => SetLocation(window, location.X, location.Y);
        #endregion
        ...
        public T SetMaximized(bool maximized) {
            ...
            window.Invoke(() => {
                ...
                if (maximized) {
                    ...
                    window.Events.InvokeOnMaximized();
                }
                    ...
                    window.Events.InvokeOnRestored();
                }
            });
            return window;
        }
        ...
        public T SetMaxSize(Size size) => SetMaxSize(window, size.Width, size.Height);

        ///<summary>Native window maximum Height in pixels.</summary>
        public T SetMaxHeight(int maxHeight) => SetMaxSize(window, window.MaxWidth, maxHeight);

        ///<summary>Native window maximum Width in pixels.</summary>
        public T SetMaxWidth(int maxWidth) => SetMaxSize(window, maxWidth, window.MaxHeight);
        ...
        public T SetMinSize(Size size) => SetMinSize(window, size.Width, size.Height);

        ///<summary>Native window maximum Height in pixels.</summary>
        public T SetMinHeight(int minHeight) => SetMinSize(window, window.MinWidth, minHeight);

        ///<summary>Native window maximum Width in pixels.</summary>
        public T SetMinWidth(int minWidth) => SetMinSize(window, minWidth, window.MinHeight);
