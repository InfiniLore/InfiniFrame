...
public class OffsetTests {
    ...
    public async Task AtWindowStage_DirectAssignment_IntOverload(CancellationToken ct) {
        ...
        int initialLeft = await PollUtility.WaitForChangeAsync(getValue: () => window.Features.Position.Left, originalLeft, TimeSpan.FromSeconds(5), ct);
        int initialTop = await PollUtility.WaitForChangeAsync(getValue: () => window.Features.Position.Top, originalTop, TimeSpan.FromSeconds(5), ct);

        // Act
        window.Features.Position.Offset(40, 30);

        // Assert
        int newLeft = await PollUtility.WaitForChangeAsync(getValue: () => window.Features.Position.Left, initialLeft, TimeSpan.FromSeconds(5), ct);
        int newTop = await PollUtility.WaitForChangeAsync(getValue: () => window.Features.Position.Top, initialTop, TimeSpan.FromSeconds(5), ct);
