...
export default function WindowFeatureTestPanel() {
    ...
    return <Container maxWidth={false} sx={{py: 4}}>
        <Stack spacing={3}>
            ...
            <Typography>Input probes send entered data to InfiniFrame. Output probes read InfiniFrame data into their
                fields.</Typography>
            <Box sx={{display: 'grid', gridTemplateColumns: {xs: '1fr', md: 'repeat(2, 1fr)'}, gap: 3}}>
                <InputDataProbe title={title} titleId="current-window-title" buttonText="Apply title / reset"
                                buttonId="title-toggle-button" dataInputId="title-data-input" dataLabel="Window title"
                                data={titleInput} onDataChanged={setTitleInput} onSubmitted={applyOrResetTitle}/>
                <OutputDataProbe title="Fullscreen" buttonText="Toggle fullscreen" buttonId="fullscreen-toggle-button"
                                 dataInputId="fullscreen-data-result" dataLabel="Current fullscreen state"
                                 data={fullscreenData} onReadRequested={toggleFullscreen}/>
            </Box>
            <Divider/>
            <Typography variant="h5">Window feature readers</Typography>
            <Box id="window-feature-test-panel" sx={{
                display: 'grid',
                gridTemplateColumns: {xs: '1fr', md: 'repeat(2, 1fr)', lg: 'repeat(3, 1fr)'},
                gap: 3
            }}>
                {probes.map(probe => <OutputDataProbe key={probe.key} title={probe.title} buttonText={probe.buttonText}
                                                      buttonId={`probe-${probe.key}-feature`}
                                                      dataInputId={`${probe.key}-feature-result`}
                                                      dataLabel="Serialized window data" data={results[probe.key]}
                                                      onReadRequested={() => readFeature(probe)}/>)}
            </Box>
        </Stack>
    </Container>
}
